...
final class BucketManager
{
    ...
    public function listFiles($bucket, $prefix = null, $marker = null, $limit = 1000, $delimiter = null)
    {
        ...
        public function getRefresh($body)
    {
        $url = Config::API_FUSION . '/refresh';
        list($ret, $error) = $this->get($url,$body);
        if ($ret === null) {
            return array(null, null, $error);
        }
        $marker = array_key_exists('marker', $ret) ? $ret['marker'] : null;
    ...
    public function refresh($bucket, $key)
    {
        $path = '/refresh/' . \Qiniu\entry($bucket, $key);
        list(, $error) = $this->rsPost($path);
    }
     ...
     */
    public function copy($from_bucket, $from_key, $to_bucket, $to_key,$force=0)
    {
     ...
     */
    public function move($from_bucket, $from_key, $to_bucket, $to_key,$force=0)
    {
        $from = \Qiniu\entry($from_bucket, $from_key);
        $to = \Qiniu\entry($to_bucket, $to_key);
    ...
    private function rrsPost($path, $body = null)
    {
        $url = Config::RS_HOST . $path;
        return $this->post($url, $body);
    }
